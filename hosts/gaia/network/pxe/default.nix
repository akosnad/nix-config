    ...
    buildPhase =
      let
        mkArchDir = arch: ''
          ...
          cp -rv --reflink=auto ${ipxes."${arch}"}/* $out/${arch}/.
          ...
          ln -vs ${installers."${arch}".config.system.build.netbootRamdisk}/initrd $out/${arch}/installer/initrd

          cat << EOF > $out/${arch}/installer/boot.ipxe
          #!ipxe
          kernel ${bootWebroot}/${arch}/installer/kernel init="${toString installers."${arch}".config.system.build.toplevel}/init" ${toString installers."${arch}".config.boot.kernelParams}
