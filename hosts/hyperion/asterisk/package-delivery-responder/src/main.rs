...
use futures_util::{future, lock::Mutex, pin_mut, stream::{Next, SplitStream}, StreamExt as _};
...
use tokio_util::{bytes::{Buf, Bytes, BytesMut}, codec::{self, BytesCodec}, udp::UdpFramed};
        ...
        async move {
            ...
            if event.data.channel.name.starts_with("UnicastRTP") {
                ...
                let ext_media = client
                    ...
                    .await.inspect_err(|e| log::error!("external media channel creation failed: {e:?}"))?;
                ...
                let Ok(remote_socket_addr) = SocketAddr::from_str(format!("{ip}:{port}").as_str()) else {
                    return Err(AriError::Internal("external media channel ip/port parse failed".to_string()));
                };
                ...
                tokio::spawn(async move {
                    ...
                    let bridge_to_rtp = bridge_to_rtp(bridge_rx, rtp_sock.clone(), remote_socket_addr);
...
async fn bridge_to_rtp(mut bridge_rx: UnboundedReceiver<Vec<u8>>, rtp_sock: Arc<UdpSocket>, remote_socket_addr: SocketAddr) {
    let mut packetizer = rtp::packetizer::new_packetizer(240, 0, 0, Box::new(G711Payloader::default()), Box::new(new_fixed_sequencer(0)), 8_000);
    while let Some(pcm_samples) = bridge_rx.next().await {
        let ulaw_samples = pcm_samples.iter().tuple_windows().map(|(high_byte, low_byte)| {
...
async fn rtp_to_bridge(tx: UnboundedSender<Bytes>, rtp_sock: Arc<UdpSocket>, remote_socket_addr: SocketAddr)  {
    loop {
        ...
        match rtp_sock.recv_from(&mut buf).await {
            Ok((n, socket_addr)) if socket_addr == remote_socket_addr => {
               ...
               match rtp::packet::Packet::unmarshal(&mut buf) {
                   Ok(packet) => {
                       ...
                       let pcm_samples = packet.payload.iter().map(|ulaw_sample| {
            ...
            Ok((_, socket_addr)) => {
                log::warn!("received unrelated data on RTP packet from {socket_addr}, when expected {remote_socket_addr}");
            }
            Err(e) => {
