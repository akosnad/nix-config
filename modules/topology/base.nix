...
in
{
  perSystem =
    { system, ... }:
    {
      checks = if system != "x86_64-linux" then { } else {
        topology = flakeConfig.flake.topology.${system}.config.output;
      };
      ...
      topology.modules = [
        {
          nodes =
            let
              localDevices = lib.filterAttrs (_n: d: d.local && !d.hidden) flakeConfig.flake.devices;
              hasEspHome = name: lib.hasAttr name flakeConfig.flake.esphomeHosts;
            in
            lib.mapAttrs
              (n: d: {
                  ...
                  details =
                    let
                      filterProtocol =
                        ...
                        builtins.filter (
                          p:
                          builtins.elem p.proto [
                            proto
                            "tcpudp"
                          ]
                        );
                      formatPorts = map (
                        p: if p.source == p.dest then toString p.dest else "${toString p.dest}:${toString p.source}"
                      );
                      mkPortForwardList =
                        proto:
                        lib.pipe d.forwardedPorts [
                ...
                services.blockInternetAccess =
                  lib.mkIf (builtins.elem true (lib.attrValues d.blockInternetAccess))
                    {
          ...
          icons =
            let
              ...
              mkCategory =
                category:
                lib.pipe (builtins.readDir "${iconsBasePath}/${category}") [
                  (lib.filterAttrs (_: v: v != "directory"))
                  (lib.mapAttrs' (
                    k: _: lib.nameValuePair (stripFileExtension k) { file = "${iconsBasePath}/${category}/${k}"; }
                  ))
                ];
            in
