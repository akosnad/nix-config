...
in
{
  config.flake.modules.nixos."hosts/gaia" = { config, ... }:
    let
      ...
      gatewayIp = flakeConfig.flake.devices.gaia.ip;
      ...
      deviceHostnames = lib.mapAttrs' (_: d: { name = d.ip; value = mapExtraHostnames d d.extraHostnames; }) flakeConfig.flake.devices;
    in
    {
      services.dnsmasq = {
        enable = true;
        settings = {
          ...
          dhcp-option = [
            ...
            "tag:lan,option:sip-server,${flakeConfig.flake.devices.hyperion.ip}"
