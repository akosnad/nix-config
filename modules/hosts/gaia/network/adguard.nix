...
in
{
  config.flake.modules.nixos."hosts/gaia" =
    { config, ... }:
    let
      localhostHostEntries = lib.filterAttrs
        (
          ip: _: ip == "::1" || ip == "127.0.0.1" || ip == "127.0.0.2"
        )
      ...
      mapExtraHostname = d: h: [
        (mkHostRewrite d.ip h)
        (mkHostRewrite d.ip "${h}.${config.networking.domain}")
      ];
    ...
    in
    {
      services.adguardhome = {
        ...
        settings = {
          ...
          dns = {
            bind_hosts = [
              flakeConfig.flake.devices.${config.networking.hostName}.ip # LAN
            ];
